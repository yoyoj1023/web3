 ...
 * @param config.toBlock - optional block number to end reading events at (helps limit eth_getLogs requests to stay under 500 block limit)
...
>({
  ...
  const query = useInfiniteQuery({
    ...
    getNextPageParam: (lastPage, allPages, lastPageParam) => {
      ...
      // If toBlock is specified, don't fetch beyond it
      if (toBlock && nextBlock > toBlock) return undefined;
