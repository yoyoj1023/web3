 ...
 * @param config.toBlock - the block number to stop reading events at (optional, if not provided will read up to latest block)
  ...
  const query = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      ...
      const data = await getEvents(
        { address: deployedContractData?.address, event, fromBlock: pageParam, toBlock, args: filters },
        publicClient,
    ...
    getNextPageParam: (lastPage, allPages, lastPageParam) => {
      const upperLimit = toBlock || blockNumber;
      if (!upperLimit || fromBlock >= upperLimit) return undefined;
      ...
      if (nextBlock > upperLimit) return undefined;
