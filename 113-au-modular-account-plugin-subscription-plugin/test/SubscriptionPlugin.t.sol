...
contract SubscriptionPluginTest is Test {
    ...
    function test_Subscribe() public {
        ...
        UserOperation memory userOp = UserOperation({
            ...
            nonce: 0,
            initCode: "",
            callData: abi.encodeCall(SubscriptionPlugin.subscribe, (service, 13)),
    ...
    function test_Collect() public {
        ...
        vm.prank(service);
        skip(4 weeks);
        subscriptionPlugin.collect(address(account1), 13);
        assertEq(service.balance, 13);

        // check that we successfully collected!
        // (uint amount, uint lastpaid, bool enabled) =  subscriptionPlugin.subscriptions(service, address(account1));
        // assertEq(amount, 0);
