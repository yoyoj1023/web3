...
⚠️ Let's revert with `Lending_InvalidAmount()` right at the start if someone attempts to use the function with the `amount` parameter set to 0. We also want to revert if the sender doesn't have the `amount` of collateral they are requesting.
...
- [ ] 🖥️ Does the front end update when you do each action?
...
🫗 Now we will fill in the details on `isLiquidatable`. This function should return a bool indicating if the position ratio is less than `COLLATERAL_RATIO`. See if you can implement the logic without the hint.
...
🏗️ Let's start by adding a couple helper methods to the `Lending.sol` contract.
